function [tou0 ,tou1 ,tou21, tou22, t21, t22 ,lam,w]=find_t_lam_w(rho,w_new)
lam=10;
t_new=10;
a=0;
n=0;
...
syms tou0 tou1 tou21 tou22 t21 t22 lam1 lam2 lam3 lam4
    ...
    eqn3=lam3*(log(1+rho(3)*t21/tou21)-(rho(3)*t21/tou21)/(1+rho(3)*t21/tou21))==lam1*log(2);
    eqn4=w(2)*(log(1+rho(3)*t22/tou22)-(rho(3)*t22/tou22)/(1+rho(3)*t22/tou22))==lam1*log(2);
    eqn5=lam3* rho(3)/(1+rho(3)*t21/tou21)==lam2*log(2);
    eqn6=w(2)* rho(3)/(1+rho(3)*t22/tou22)==lam2*log(2);
    ...
    if isempty(t_new)||sum(w<0)%||isempty(lam)
        w=[abs(w_new(1)*rand())*1.2 ,abs(w_new(2)*rand())*1.2]
    else
