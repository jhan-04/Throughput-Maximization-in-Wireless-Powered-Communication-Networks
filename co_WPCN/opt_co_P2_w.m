function [t, R] = opt_co_P2_w(rho)


% wi=[100 100;20 20;15 15;7 7;1 1];%P0=0P0=5P0=10P0=15

w=[100/log(1+rho(1)) ,100/log(1+rho(3))]
%w=[100/log(1+(rho(1)+rho(2))/2) ,100/log(1+rho(3))]
%w=[10 10]
R_max=4;
R_min=0;
w_new=w;
...
while R_max - R_min >0.00001
    ...
    [tou0 ,tou1 ,tou21, tou22, t21, t22,lam,w]=find_t_lam_w(rho,w_new);
    ...
    %  R1_co= min(tou1*log2(1+rho(1)*tou0/tou1)+tou21*log2(1+rho(3)*t21/tou21),tou1*log2(1+rho(2)*tou0/tou1));
    ...
    if g_w > 0 %infeasible%%(5)
       ...
       elseif g_w <=0 %feasible
        ...
        f_min=1000;
        f_min_new= min(f_w_new,f_w);
        a = 0;
        st=0;
        g = R - R_m;
        % while abs(f_min_new - f_min) >abs(0.001*f_min)%(5)update lambda
        while abs((f_w_new - f_w)) >abs(0.001*f_w) && abs((f_w_new - f_w)) >0.0001&&f_w_new>0
            if  (w -st * g)~=w_new
                a=a/2;
            end
            ...
            w = w_new;
            a = a + 1;
            st=0.1/sqrt(a+1);%%step size
            
            f_w = f_w_new;
            g = R - R_m; %%subgradient of -g(lambda)
            w_new = w -st * g; %x(k+1)=x(k)-a*g(k)
            
            
            [tou0 ,tou1 ,tou21, tou22, t21, t22,lam ,w_new]=find_t_lam_w(rho,w_new);
            t= [tou0 ,tou1 ,tou21, tou22, t21, t22];
